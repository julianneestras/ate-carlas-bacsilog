...
if(isset($_POST["checkout"]))
{
  include 'conn.php';
    ...
    $food_total =  $_POST['food_total'];
    $orderid =  $_POST['orderid'];
    $prodno =  $_POST['prodno'];

    $_SESSION['Food_ID']=$_POST['Food_ID'];

    header("location: UserCheckout.php?Province=$Province&City=$City&Barangay=$Barangay");

    /*sql = "INSERT INTO ordersummary (Order_ID, ProdNo, ProdName, ProdPrice, ProdQty,Total,SubTotal) VALUES ('$orderid', '$prodno', '$prodname', '$prodprice' , '$prodqty', '$prod_total',
           '$food_total')";
...
if(isset($_POST["add_to_cart"]))
{
  ...
  $prodprice = $_POST['prodprice'];
  $sql = "INSERT INTO ordersummary (Username,ProdName,ProdNo,ProdQty,ProdPrice) VALUES ('$u_Username','$prodname','$prodno','$prodqty','$prodprice')";
if(mysqli_query($conn, $sql)){
    echo "Records added successfully.";
} else{
    echo "ERROR: Could not able to execute $sql. " . mysqli_error($conn);
  ...
  if(isset($_SESSION["shopping_cart"]))
  {
    ...
    else
    {
      ...
      echo '<script>window.location="UserOrderForm.php"</script>';
    }
  }
  else
  {
...
<div class="container-fluid  py-3" style="background-color: #1c1c1c;">
  <div class="row sticky-top">
    ...
    <div class="col-xl-lg-4 col-lg-4 col-md-4 col-sm-4 d-flex align-items-center justify-content-center pe-3 pt-1">
      ...
      <a class="a content" href="index.php" style="text-decoration: none; cursor: pointer;"> Profile </a> &nbsp; 
                
                  <input type="submit" name="logout" value="LOGOUT" style="background: linear-gradient(to right, #13868A 0%, #0A6668 100%); width: 100%; color: white">
      ...
      <div class="tab-content" id="pills-tabContent">
       ...
       $result = mysqli_query($connect, $query);
          ...
          while($row = mysqli_fetch_array($result))
        {
                ...
                echo '
                    <form method="post" action="UserOrderForm.php?action=add&id='.$Food_ID.'">
                         <div class="card product" style="width: 15rem; box-shadow: rgba(50, 50, 93, 0.25) 0px 6px 12px -2px, rgba(0, 0, 0, 0.3) 0px 3px 7px -3px;" style="display: inline-block;">
                            ...
                            <div class="card-body">
                                ...
                                <input type="hidden" name="prodno" value="<?php echo $row["Food_ID"]; ?>" />
                                <input type="hidden" name="prodname" value="<?php echo $row["Food"]; ?>" />
                                <input type="hidden" name="prodprice" value="<?php echo $row["Price"]; ?>" />

                               <input type="hidden" name="hidden_name" value="<?php echo $row["Food"]; ?>" />

                               <input type="hidden" name="hidden_price" value="<?php echo $row["Price"]; ?>" />
                                <input type="hidden" name="u_Username" <?php echo "value='".$_COOKIE["name"]."'"; ?> >
    ...
    <div class="div2"> 
      <div class="cart-container">
        <h4>Your Cart <span class="price" style="color:black"><i class="fa fa-shopping-cart"></i></span></h4>
          ...
          ?>
          <tr >
            <td><input type="textbox" size="10" name="prodname" value="<?php echo $values["item_name"]; ?>" readonly></td>
            <td><input type="textbox" size="2" name="prodqty" value="<?php echo $values["item_quantity"]; ?>" readonly></td>
            <td><input type="textbox" size="6" name="prodprice" value="₱ <?php echo $values["item_price"]; ?>" readonly></td>
            <td><input type="textbox" size="6" name="prod_total" value="₱ <?php echo number_format($values["item_quantity"] * $values["item_price"], 2);?>" readonly></td>
            <td><a href="UserOrderForm.php?action=delete&id=<?php echo $values["item_id"]; ?>"><span class="text-danger">Remove</span></a></td>
          </tr>
          <?php
          ...
          <tr>
            ...
            <td align="right"><input type="textbox" size="6" name="food_total" value="₱ <?php echo number_format($total, 2); ?>" readonly></td>
            <td></td>
          </tr>
          <?php
          }
      ...
      <button class="button-checkout" type="submit" name="checkout">Check Out</button>
      </form>
    </div>
   </div>
</div>
...
if (isset($_POST['checkout'])){
  ...
  $_SESSION['prodqty']=$_POST['prodqty'];
  $_SESSION['prodprice']=$_POST['prodprice'];
  $_SESSION['prod_total']=$_POST['prod_total'];
  $_SESSION['food_total']=$_POST['food_total'];
